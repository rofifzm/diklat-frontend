    ...
    <div class="container">
        <div class="card p-2 d-flex flex-col text-center shadow rounded ">
            ...
            <div class="mb-3 d-flex flex-row">
                <select v-if="!ulangButton" class="form-control me-2" name="" id="" @change="ubahLokasi" v-model="lokasi">
                    ...
                    <option v-else :value="[kec['id'], kec['kec']]">{{kec['kec']}}</option>
                </select>
                <button v-if="inputOptions.data['kecamatan'] && ulangButton == false" type="button"  @click="submitLokasi" class="btn btn-primary">Submit</button>
                <button v-else type="button" class="btn btn-primary" :class="{'w-100': ulangButton}" @click="aturUlang" >Reset</button>
            </div>
        </div>
